...
const Navigation: FC<INavigation> = ({
  ...
  return (
    <div className={cn(styles.navigation)} onClick={() => setIsOpen(!isOpen)} ref={menuRef}>
      <AiOutlineEllipsis />
      <div className={cn(styles.modal, { '!block': isOpen })}>
          ...
          <div className={styles.modalWrapper}>
            ...
            <div className={styles.modalBlock}>
              {playlists.data?.map((playlist, i) => (
                <div key={i} onClick={() => mutateAsync(playlist._id)} aria-label="Add playlist">
                  <p>
                    <span>{i + 1}. </span>
                    {playlist.name === 'My playlist' ? t(playlist.name) : playlist.name}
                  </p>
                </div>
              ))}
            </div>
        ...
        {isEdit && (
          <>
            {handleDelete && (
              <button onClick={() => handleDelete(id || '')} aria-label="Delete playlist">
                <p>{t('Delete playlist')}</p>
              </button>
            )}
            <button onClick={() => openModal && openModal(true)} aria-label="Change information">

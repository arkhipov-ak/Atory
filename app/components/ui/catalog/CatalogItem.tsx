    ...
    <li onDoubleClick={handlePlayAudio}>
      <div className={styles.playWrapper}>
        {isPlaying && _id === activeId && activeIndex === index ? (
          <Image src={EqualizerGif} width={20} height={20} alt="Equaliser" draggable={false} />
        ) : (
      ...
      <div>
        <Image src={poster} width={50} height={50} alt="track cover" draggable={false} />
        <div>
          <p>{title}</p>
          <p>
      ...
      <div>
        ...
        <p>{fmtMSS(duration)}</p>
        <Navigation id={_id} author={author} album={album} playlistId={playlistId} />
      </div>
    </li>
  )
}

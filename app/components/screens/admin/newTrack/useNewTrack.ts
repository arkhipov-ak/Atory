...
import { IAlbumCreate, IAuthorCreate, ITrackCreate } from '@/shared/types/track.types'
...
export const useNewTrack = () => {
  ...
  const { mutateAsync: mutateAsyncAuthor } = useMutation('create author', (data: IAuthorCreate) =>
    AuthorService.create(data),
  )

  const { mutateAsync: mutateAsyncAlbum } = useMutation('create album', (data: IAlbumCreate) =>
  ...
  const { data: authors } = useQuery('get author list', () => AuthorService.getAll(), {
    select: ({ data }) =>
      data.map((author) => {
        return {
          value: author.title,
  ...
  const { data: albums } = useQuery('get album list', () => AlbumService.getAll(), {
    select: ({ data }) =>
      data.map((album) => {
        return {
          value: album.title,

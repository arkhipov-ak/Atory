...
export const usePlaylist = (setIsOpen?: (arg: boolean) => void, id?: string) => {
  ...
  const queryData = useQuery('playlist list', () => UserService.getPlaylists(), {
    select: ({ data }) =>
      data.map((playlist) => ({
        ...playlist,
  ...
  const {
    ...
    isLoading,
    refetch,
  } = useQuery(['playlist', query.id], () => PlaylistService.getById(String(query?.id)), {
